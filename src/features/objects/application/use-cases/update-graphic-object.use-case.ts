...
import { UpdateGraphicObjectCommand, GraphicObjectResponseDto } from '../dto/index.js';
...
@Injectable()
export class UpdateGraphicObjectUseCase implements UseCase<UpdateGraphicObjectCommand, GraphicObjectResponseDto> {
  constructor(
  ...
  async execute(command: UpdateGraphicObjectCommand): Promise<GraphicObjectResponseDto> {
    ...
    await this.events.publish({ type: 'object.updated', objectId: saved.id, occurredAt: new Date(), data: { width: saved.width, height: saved.height } });

...
import { DeleteGraphicObjectCommand, DeleteGraphicObjectResponseDto } from '../dto/index.js';
...
@Injectable()
export class DeleteGraphicObjectUseCase implements UseCase<DeleteGraphicObjectCommand, DeleteGraphicObjectResponseDto> {
  constructor(
  ...
  async execute(command: DeleteGraphicObjectCommand): Promise<DeleteGraphicObjectResponseDto> {
    ...
    await this.events.publish({ type: 'object.deleted', objectId: command.objectId, occurredAt: new Date() });
    return { ok: true };
  }
}
